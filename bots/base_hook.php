...
if (
  ...
  // Прошу chatGPT запропонувати кейси того, що можна робити тут 
  // (вітатись з новим користувачем, надсилати йому правила в особисті тощо)
...
// Прошу chatGPT вказати які кейси пропущені (через що може виникати ситуація empty($chat) and empty($user)) і запропонувати рішення
...
// це чернетка яка дуже НЕДОРОЗРОБЛЕНА
// chatGPT мені не подобається як тут формується профайл, дуже не наглядно та незручно і це при тому що профайл поки не великий і поки лише українською. Страшно уявити яка тут буде простиня коли профайл збільшиться, тож треба покращити вже
if ($textCommand == true and $text === '/profile') {
  ...
  if ($bundle['dopphone'] != '') {
    ...
    $dopphonev = "_ЩЕ НЕ ВКАЗАНО_";
    $eddopph = "вказати";
    }
  $sprob = "0";
  $response = "";
  // Перша кнопка
  $bot->insertButton([['text' => $edph . " номер телефону", 'callback_data' => 'rear_editphone']]);

  $response .= "📱 Номер телефона: " . $phonev . "\r\n---------\r\n";
  if ($bundle['role'] == '' or $bundle['role'] == 'candidate') {
    $response .= "🏠 Місто знаходження:" . $regionv . "\r\n---------\r\n";
    $response .= "🆘 Замовлень " . $botinfodop['service'] . ": " . $sprob;
    }
  if ($bundle['role'] == 'candidate') {
    $bot->insertButton([['text' => "скасувати статус кандидата", 'callback_data' => 'rear_addserviceno']]);
    $response .= "\r\n---------\r\nРеєстрація у якості " . $botinfodop['service'] . ":\r\n_КАНДИДАТ_";
    }
    // Якщо це надавач послуг
  if ($bundle['role'] == 'service') {
    ...
    // Можна додавати тільки після того як ми його внесли як адвоката
    if (!empty($service)) {
    ...
    $response .= "Успішно опрацьовано замовлень: *0*"; // Поки що функціонал відстеження кількості замовлень в розробці
    } else {
    $bot->insertButton([['text' => "змінити регіон знаходження", 'callback_data' => 'rear_editregion']]);
    }
  $bot->insertButton([["text" => $gotostart, "callback_data" => "/start"]]); // Повернутись назад
...
// А знач далі user_key нормально працює
  ...
  // Тут ніби трохи каша
  // Прошу chatGPT довести до робочого стану якщо є прогалини в логіці
  if ($typelink == 'a') {
