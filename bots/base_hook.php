...
$partner = false;       // Повна анкета користувача сайту (якщо бот працює в зв'язці з сайтом) $botinfo["type"] != bot
...
$groupsetup = false;    // Додаткові налаштування публічного чату в якому відбувається діалог
...
Коли бот є частиною сайту, то до прикладу 
він може мати веб інтерфейс або може взаємодіяти з базою користувачів сайту
...
! Але змінивши $botinfo["type"] на !=bot потрібно зробити дебагінг коду
! Бо система денеде почне шукати load::$partners для отримання додаткової інформації по користувачу
*/
if ($st == 'bot') {
...
// Змінювати це на роботу з кількома країнами поки що не планую
...
$aco = $botinfo["country"];
$lg = $botinfo["country"];  // Для мови ви можете створити окремий стовбець
...
if ($isAdmin !== true and $isUsher !== true and ($chattype == 'group' or $chattype == 'supergroup')) {
  ...
  if ($urlInText == true) {
    ...
    if ($bot->needsend == 'needsend') {
      ...
      // за повторне порушення в такий короткий термін
      // Також можна зберігати в tg_bundle признак status = black 
      // where bot = $botinfo['name'] (В рядку з цим ботом) або й навіть у всіх ботах системи
      // якщо саме цей бот використовується в багатьох чатах, то він тримає в фокусі користувача в усіх чатах
      // Тому нажаль використати цей метод до одного конкретного чату не можливо, 
...
if (!empty($bundle['mst']) and $bundle['mst'] == 'stop') {
  tg::UpdateTgBundle($user, $bundle['email']);
  }
...
// Якщо видалився учасник
  if (isset($data['message']['new_chat_members']) or isset($data['message']['left_chat_member'])) {
    ...
    // В чатах куди користувачі активно вступають та з яких активно видаляються 
    // і в яких при цьому мало спілкуються, ці повідомлення бісять і змушують користувачів мьютити чат
    // АЛЕ через базу даних можна вимикати цю можливість
    if (!empty($groupsetup['delInOut'])) {
...
if ($callback_data) {
  ...
  if (stristr($callback_data, 'choose_') !== false) {
    ...
    tg::selectCity($bot, $bundle, $list, $arr, $response);
    exit('ok');

    } // choose_


  // Якщо клієнт натиснув на кнопку, яка не описана вище
  // Виходимо не обробивши клік
  // ! Можливо про це треба повідомляти адміна чи розробника
...
// та поступила команда на реєстрацію чи замовлення (купівлю)
if ($textCommand == true and ($text === '/reg' or $text == '/buy')) {
...
// В залежності від цього змінюється відповідь
  ...
  // Отож перед нами безпосередньо замовлення $text == '/buy'

  // Якщо кліент раніше вказував регіон, то ніяких проблем
